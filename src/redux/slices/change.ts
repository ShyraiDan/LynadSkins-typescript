...
const changeSlice = createSlice({
  ...
  reducers: {
    addChange: (state, action: PayloadAction<TSkin>) => {
      const indx = state.marketItems.findIndex((item) => item._id === action.payload._id)
      const index = state.yourItems.findIndex((item) => item._id === action.payload._id)
    ...
    removeChange: (state, action: PayloadAction<string>) => {
      state.marketItems = state.marketItems.filter((item) => item._id !== action.payload)
      state.yourItems = state.yourItems.filter((item) => item._id !== action.payload)
    },
    resetChange: (state) => {
      state.marketItems = []
      state.yourItems = []
