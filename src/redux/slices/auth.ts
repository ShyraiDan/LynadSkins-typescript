...
export const fetchAuth = createAsyncThunk('auth/fetchAuth', async (params: TfetchAuthparams) => {
  const { data } = await axios.post('/auth/login', params)
  return data
})

export const fetchAuthMe = createAsyncThunk('auth/fetchAuthMe', async (params: string) => {
  const { data } = await axios.get('/auth/me', { data: params })
  return data
})

export const fetchRegister = createAsyncThunk('auth/fetchRegister', async (params: TfetchRegisterparams) => {
  const { data } = await axios.post('/auth/register', params)
  return data
})

export const updateUser = createAsyncThunk('auth/updateUser', async (params: TUser) => {
...
const authSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder
      ...
      .addCase(fetchRegister.fulfilled, (state, action: PayloadAction<TUser>) => {
        state.status = 'loaded'
        state.data = action.payload
      })
