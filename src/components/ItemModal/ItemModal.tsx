...
export const ItemModal: FC<IItemModal> = ({ data, addToCart, page, handleTrade, status }) => {
  ...
  return (
    <Container styles={styles.container}>
      <div className={styles['modal-container']}>
        <img src={data?.imageUrl} alt={`${data?.itemName}-${data?.skinName}`} className={styles['modal-image']} />
      </div>
        ...
        {!page && (
          <div className={styles['modal-buttons']}>
            <Button
              ...
              text={page ? (status ? t('remove_sale') : t('sale')) : t('add_cart')}
              style={`${styles['modal-cart']} ${indexCart !== -1 && styles['modal-cart_added']}`}
            ...
            {!page && (
              <Button
                ...
                text={<FaHeart fill={indexWish !== -1 ? '#e71fb8' : 'white'} fontSize={'30px'} />}
                onClick={() => addToWish()}
