    ...
    <Container>
      <div className={styles.item} onClick={() => handleClick()}>
        <div className={styles.left}>
          <img src={data.imageUrl} alt={`${data.itemName}-${data?.skinName}`} className={styles.photo} />
          <div className={styles.description}>
            ...
            <Typeography fontSize={'12px'} color={'grey'}>
              <Typeography variant={'span'} color={'gold'} fontSize={'12px'}>{`${
                data.souvenir === true ? 'SV ' : ''
              }`}</Typeography>
              <Typeography variant={'span'} fontSize={'12px'}>{`${data.souvenir === true ? '/ ' : ''}`}</Typeography>
              <Typeography variant={'span'} fontSize={'12px'} color={'orange'}>{`${
            ...
            <Typeography color={'white'} fontSize={'16px'}>{`$ ${data.price.toFixed(2)}`}</Typeography>
          </div>
        </div>
        {type && (
