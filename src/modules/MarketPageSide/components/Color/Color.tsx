...
export const Color: FC<IColor> = ({ register }) => {
  ...
  return (
    <MContainer styles={styles.typeContainer} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          ...
          return (
            <>
              <label
                ...
                onClick={() => handleSelected(item.val)}
              ></label>
              <input id={item.val} type='checkbox' value={item.val} {...register('color', {})} hidden />
            </>
          )
        })}
      </List>
