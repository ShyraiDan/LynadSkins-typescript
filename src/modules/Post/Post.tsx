...
export const Post: FC<IPost> = ({ data }) => {
  ...
  return (
    <Container styles={styles.container}>
      {data ? (
        <>
          <img src={`${process.env.REACT_APP_API_URL}${data.imageUrl}`} alt='' className={styles.postImage} />
            ...
            <div className={styles.userInfo}>
              ...
              <div className={styles.info}>
                <div className={styles.userDetails}>
                  ...
                  <span className={styles.additional}>{data?.createdAt.substring(0, 10)}</span>
                </div>
                {userData?._id === data?.user._id && (
                  <div>
                    ...
                    <FaTrash fill='var(--text-primary)' onClick={onClickRemove} className={styles.remove} />
                  </div>
                )}
              </div>
            </div>
            <div className={styles.postContainer}>
              <Typeography color={'white'} variant={'h1'} fontSize={'32px'} fontWeight={800}>
                {data?.title}
              </Typeography>
