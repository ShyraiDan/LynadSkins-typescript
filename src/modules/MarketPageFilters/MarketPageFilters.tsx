...
import { FaAngleDown, FaFilter, FaHeart, FaSearch, FaShoppingCart } from 'react-icons/fa'
    ...
    <div className={styles.filters}>
      ...
      <div className={styles['filter-bottom']}>
        <div className={styles['filter-left']}>
          <motion.div whileTap={{ scale: 0.8 }} className={styles.fav} onClick={() => openFavMenu()}>
            <div className={styles['icon-heart']}>
              <FaHeart fontSize={'15px'} fill='white' />
              {wishList.length > 0 && <div className={styles['icon-heart__number']}>{wishList.length}</div>}
            </div>
          </motion.div>
          <div className={styles['filter-box']} onClick={showAll}>
            {filter}
            <div className={open ? `${styles.icon} ${styles['icon-opened']}` : styles.icon}>
              ...
              <div className={styles['filters-container']}>
                <List>
                  {filterParams.map((item, i) => {
                    return (
                      <ListItem key={i} style={styles.item} onClick={() => filterModalClick(item)}>
                        {item}
                      </ListItem>
                    )
                  })}
        ...
        <div className={styles['filter-right']}>
          <motion.div whileTap={{ scale: 0.8 }} className={styles.search} onClick={() => searchModalClick()}>
            <div className={styles['icon-search']}>
              <FaSearch fontSize={'15px'} fill='white' />
            </div>
          </motion.div>
          {page && (
            <motion.div whileTap={{ scale: 0.8 }} className={styles.cart} onClick={() => openCartMenu()}>
              <div className={styles['icon-cart']}>
                <FaShoppingCart fontSize={'15px'} fill='white' />
                {cart.length > 0 && <div className={styles['icon-cart__number']}>{cart.length}</div>}
              </div>
            </motion.div>
          )}

          <motion.div whileTap={{ scale: 0.8 }} className={styles['icon-filters']} onClick={() => handleClick()}>
            <FaFilter fontSize={'15px'} fill='white' />
          </motion.div>
        </div>
      </div>
      <AnimatePresence initial={false}>
        {moreFilters && <Modal handleClick={() => handleClick()} children={<FilterPhones />} guns={true} />}
      </AnimatePresence>
      <AnimatePresence initial={false}>
        {cartOpen && <ModalSmall handleClick={() => openCartMenu()} children={<Cart setStateModal={setCartOpen} />} />}
      </AnimatePresence>
      <AnimatePresence initial={false}>
        {favOpen && <ModalSmall handleClick={() => openFavMenu()} children={<Fav />} />}
      </AnimatePresence>
      <AnimatePresence initial={false}>
        {search && <Modal handleClick={() => searchModalClick()} children={<Search />} guns={true} />}
      </AnimatePresence>
    </div>
  )
}
