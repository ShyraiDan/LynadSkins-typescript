  ...
  return (
    <>
      <Container styles={styles.container}>
        <motion.div initial='hidden' whileInView='visible' viewport={{ once: true }} className={styles.top}>
          {knives.map((item, i) => (
            <Knife key={i} data={item} />
          ))}

          <div className={styles.title}>
            <MTypeography color={'white'} variants={textAnimation} custom={1} variant={'h1'}>
              {t('main_title')}
            </MTypeography>
          </div>
        </motion.div>
