  ...
  return (
    <Container styles={styles.container}>
      ...
      <form action='' onSubmit={handleSubmit(onSubmit)} className={styles.form}>
        ...
        <div>
          {errors?.itemName && <p className={styles.error}>{errors?.itemName?.message || 'Mininum length equals 3'}</p>}
        </div>
        ...
        <div>
          {errors?.skinName && <p className={styles.error}>{errors?.skinName?.message || 'Mininum length equals 3'}</p>}
        </div>
        <div className={styles.selects}>
          <Controller
            ...
            render={({ field: { onChange }, fieldState: { error } }) => (
              <>
                <select className={styles.select} onChange={(newValue) => onChange(newValue)}>
                ...
                <div>{error && <p className={styles.error}>{error.message}</p>}</div>
              </>
            )}
          />
          <Controller
        ...
        <div>{errors?.price && <p className={styles.error}>{errors?.price?.message || 'Price is required'}</p>}</div>
        ...
        <div>{errors?.float && <p className={styles.error}>{errors?.float?.message || 'Float is required'}</p>}</div>

        <div className={styles.checkboxes}>
          <label htmlFor='statTrack' className={`${styles.label} ${styles['label-checkbox']}`}>
            Statrack:
          </label>
          <input id='statTrack' type='checkbox' placeholder={'statTrack'} {...register('statTrak', {})} />

          <label htmlFor='souvenir' className={`${styles.label} ${styles['label-checkbox']}`}>
            Souvenir:
          </label>
          <input type='checkbox' id='souvenir' placeholder={'souvenir'} {...register('souvenir', {})} />
        </div>
